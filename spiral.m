...
robot.plot(zeros(1,numJoints)); hold on;
...
for ii = 1 : nPts
    ...
    currentTargetPos = path(:,ii);
    

    while norm(currentTargetPos - currentPos) > 1e-3
        J = jacoba(S,M,currentQ);
        % Compute the error between the current and desired end-effector poses
        error = currentTargetPos - currentPos;
    ...
    if norm(currentTargetPos - currentPos) < 1e-3 
        disp("IK Solution converged")
    end
